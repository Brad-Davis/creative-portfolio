...
const projects = [
  ...
  { 
    ...
    // details: 'We focused on using innovative algorithms to improve efficiency. The project is widely appreciated for its impact on scalability.',
          ...
          >
            ...
            <div className="project-text">
              ...
              {expandedProjectId === project.id && project.links && (
                <>
                  <h5 style={{marginBottom: 0}}>Links:</h5>
                  <ul>
                  {project.links.map((link) => (
                    <li><a className='project-links' href={link.link} target="_blank" rel="noopener noreferrer" key={link.text}>{link.text}</a></li>
                  ))}
                   </ul>
                </>
              )}
